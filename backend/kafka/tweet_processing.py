...
class TweetProcessorProducer:
    ...
    def connect(self) -> None:
        logger.info(
            f"Connection to {self._db_connection_handler_host}:{self._db_connection_handler_port}")
        self._producer = Producer(self._conf)
        logger.info("Connection is established")

    def send(self, data: Any) -> None:
        self._producer.produce(self._send_topic, key='1',
...
class TweetProcessorConsumer:
    ...
    def models(self) -> Dict[str, BaseModel]:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            ner_model_name, **tokenizer_kwargs)
        tokenizer.add_tokens(["{{URL}}", "{{USERNAME}}", "{@", "@}"])

        ner_model = self.create_onnx_session(self.models_config['ner']['path'])
        ner_model = NerModel(ner_model, tokenizer,
                             self.models_config['ner']['id2label'])

        sent_model_name = "cardiffnlp/twitter-roberta-base-sentiment-latest"
        tokenizer = AutoTokenizer.from_pretrained(
            sent_model_name, **tokenizer_kwargs)
        sent_model = self.create_onnx_session(
            self.models_config['sentiment']['path'])
        ...
        class_model = ClassificationModel(
            class_model, tokenizer, self.models_config['category']['id2label'])
    ...
    def connect(self) -> None:
        try:
            logger.info(
                f"Connection to {self._tweet_processor_host}:{self._tweet_processor_port}")
